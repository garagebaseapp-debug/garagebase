...
export default function VnosServisa() {
  ...
  const shrani = async () => {
    if (!km || !opis) {
      setMessage('Km in opis sta obvezna!')
      return
    }
      ...
      <div className="bg-[#0f0f1a] border border-[#1e1e32] rounded-2xl p-6 flex flex-col gap-4">
        ...
        <div>
          ...
          <textarea value={opis} onChange={e => setOpis(e.target.value)}
            placeholder="npr. Menjava motornega olja + filter"
            rows={3}
        ...
        <div>
          ...
          <input type="text" value={servis} onChange={e => setServis(e.target.value)}
            placeholder="npr. Volvo Center Ljubljana"
            className="w-full bg-[#13131f] border border-[#1e1e32] rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-[#f59e0b] transition-colors" />
        </div>

        <div>
          <label className="text-[#5a5a80] text-xs uppercase tracking-wider mb-2 block">Cena (€) (po želji)</label>
          <input type="number" step="0.01" value={cena} onChange={e => setCena(e.target.value)}
            placeholder="npr. 320"
            className="w-full bg-[#13131f] border border-[#1e1e32] rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-[#f59e0b] transition-colors" />
        </div>
