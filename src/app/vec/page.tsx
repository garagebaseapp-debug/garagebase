...
export default function VecPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.warn('[GarageBase more] load failed', err)
        setVehicleCount(0)
      }
    }
    load()
  ...
  const items: MenuItem[] = [
    ...
    { title: tx('Varnostna kopija', 'Backup'), text: tx('Prenesi svoje podatke in nastavi redni opomnik.', 'Download your data and set a regular reminder.'), href: '/varnostna-kopija', icon: 'shield', tone: 'text-[#3ecfcf] bg-[#3ecfcf14]' },
