  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: servisData } = await supabase
        .from('service_logs').select('*').eq('car_id', carId)
  ...
  return (
    <div className="min-h-screen bg-[#080810] px-4 py-6 max-w-md mx-auto pb-24">
      ...
      <div className="bg-[#0f0f1a] border border-[#6c63ff44] rounded-2xl p-6 mb-4">
        ...
        {strosekNaKm && (
          <p className="text-[#5a5a80] text-sm">{strosekNaKm} €/km · {kmPrevozeni.toLocaleString()} km skupaj</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {gorivo.slice(0, 3).map((v) => (
          <div key={v.id} className="bg-[#0f0f1a] border border-[#1e1e32] rounded-xl p-4 flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white text-sm font-semibold">{v.datum}</p>
        ...
        {gorivo.length === 0 && servisi.length === 0 && (
          <div className="bg-[#0f0f1a] border border-[#1e1e32] rounded-2xl p-6 text-center">
            ...
            <p className="text-[#5a5a80] text-sm">Dodaj gorivo ali servis</p>
          </div>
        )}
      </div>
