...
export default function BackupPage() {
  ...
  const downloadBackup = async () => {
    ...
    try {
      ...
      const [fuelLogs, serviceLogs, expenses, reminders, transfers, tireSets] = carIds.length ? await Promise.all([
      ...
      setSettings(next)
      localStorage.setItem(STORAGE_KEY, JSON.stringify(next))
      setSummary({ cars: cars.length, rows: fuelLogs.length + serviceLogs.length + expenses.length + reminders.length + tireSets.length })
