...
export default function NastavitveAvta() {
  ...
  const naloziSliko = async (e: any) => {
    ...
    await supabase.from('cars')
      .update({ slika_url: urlData.publicUrl })
      .eq('id', avto.id)
  ...
  const shrani = async () => {
    ...
    // Varovalka za km — če se km zmanjša ali je velika sprememba
    const stariKm = avto.km_trenutni || 0
    const noviKm = parseInt(kmNovo) || stariKm
    const razlikaKm = noviKm - stariKm

    // Zabeležimo spremembo km če je bila
    if (noviKm !== stariKm) {
      await supabase.from('expenses').insert({
        car_id: avto.id,
        datum: new Date().toISOString().split('T')[0],
        kategorija: 'km_sprememba',
        opis: `Km posodobljeni: ${stariKm.toLocaleString()} → ${noviKm.toLocaleString()} km. Razlog: ${kmOpomba || 'Ni naveden'}`,
        znesek: 0,
      })
    }
    ...
    if (error) {
      ...
      setAvto({ ...avto, km_trenutni: noviKm })
    }
    setSaving(false)
  }
  ...
  return (
    <div className="min-h-screen bg-[#080810] px-4 py-6 max-w-md mx-auto pb-24">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => window.location.href = `/dashboard?car=${avto?.id}`}
      ...
      <div className="bg-[#0f0f1a] border border-[#1e1e32] rounded-2xl overflow-hidden mb-4">
        {avto?.slika_url ? (
          ...
          <label className="flex flex-col items-center justify-center h-36 cursor-pointer hover:bg-[#13131f] transition-colors">
            ...
            <span className="text-[#5a5a80] text-sm font-semibold">
              {uploadingSlika ? 'Nalaganje...' : 'Dodaj sliko avta'}
            </span>
      ...
      <div className="bg-[#0f0f1a] border border-[#1e1e32] rounded-2xl p-5 flex flex-col gap-4 mb-4">
        ...
        <div>
          ...
          <input value={vin} onChange={e => setVin(e.target.value)} placeholder="17-mestna VIN koda"
            maxLength={17}
      ...
      {/* KM — posebna sekcija z varovalko */}
      <div className="bg-[#0f0f1a] border border-[#f59e0b33] rounded-2xl p-5 flex flex-col gap-4 mb-4">
        ...
        {kmNovo && parseInt(kmNovo) !== avto?.km_trenutni && (
          <div>
            <label className="text-[#5a5a80] text-xs uppercase tracking-wider mb-2 block">
              Razlog spremembe <span className="text-[#f59e0b]">*</span>
            </label>
            <input value={kmOpomba} onChange={e => setKmOpomba(e.target.value)}
              placeholder="npr. Napaka pri vnosu, servis, prevzem avta..."
              className="w-full bg-[#13131f] border border-[#f59e0b44] rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-[#f59e0b] transition-colors" />
          </div>
        )}
      </div>
