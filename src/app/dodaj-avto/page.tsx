...
export default function DodajAvto() {
  ...
  const shrani = async () => {
    ...
    else { setMessage('✅ Avto uspešno shranjen!'); setTimeout(() => window.location.href = '/garaza', 1000) }
    setLoading(false)
  }
  ...
  return (
    <div className="min-h-screen bg-[#080810] px-4 py-6 max-w-md mx-auto pb-24">
      ...
      <div className="bg-[#0f0f1a] border border-[#1e1e32] rounded-2xl p-6 flex flex-col gap-4">
        ...
        {message && (
          <div className={`p-3 rounded-xl text-sm border ${message.includes('✅') ? 'bg-[#16a34a22] border-[#16a34a44] text-[#4ade80]' : 'bg-[#6c63ff22] border-[#6c63ff44] text-[#a09aff]'}`}>
            {message}
          </div>
        )}
